  ...
  switch (action.type) {
    ...
    case FILTER_DATA:
      ...
      filteredCountries = filteredCountries.filter(
        (country) =>
          country.region.toLowerCase() === action.region.toLowerCase()
      );
      return {
        ...state,
    ...
    case FILTER_SEARCHED:
      ...
      filteredSearched = filteredSearched.filter(
        (country) =>
          country.region.toLowerCase() === action.region.toLowerCase()
      );
      return {
        ...state,
        filtered: [],
        searchedFiltered: filteredSearched,
        noMatches: !filteredSearched.length && true,
      };
    case SEARCH_COUNTRIES:
      let searched = [...state.countries];
      searched = searched.filter((item) =>
        item.name.common.toLowerCase().includes(action.phrase)
      );
      return {
        ...state,
        filtered: [],
        searched,
        searchedFiltered: [],
        noMatches: !searched.length && true,
      };
    case SHOW_ALL:
      return {
        ...state,
